...
public class SunSpotHostApplication {
    ...
    // Agentes/Aplicaciones que se han registrado en la plataforma/aplicaci�n
    // <Identificador de la aplicaci�n,Socket de comunicaciones>
    ...
    public void run() {
        ...
        if(ssv == null) ssv = new SolAgentPlatformView(this);
        //ssv.setVisible(true);
...
//Methods about connections -agents and groups-    
    protected void registerConnection(String id,SenderConnection c){   
        //System.out.println("[SOL] Se ha registrado la conexi�n con el id: "+id);
...
//Revisar la conexion de grupos de SunSpot
    ...
    protected void unregisterConnection(String id){
        //System.out.println("[SOL] La conexi�n "+id+" se ha eliminado de la plataforma.");
...
//Methods about services    
    protected void registerService(String type,Service ser){
        ...
        //        " del tipo "+type+" cuyo due�o es "+ser.getOwner());
        ...
        if(_services.containsKey(type)){
            ...
            while(!end && i<ser.size()){
                if(ser.get(i).getName().equals(nameService)){
                    ...
                    //System.out.println("[SOL] Se  ha eliminado el servicio "+nameService+" cuyo due�o es "+owner
                    //        +" y el tipo es "+type);
                }
                i++;
            }
            ...
            /*NOTA
                Hemos a�adido opcionalmente esto para eliminar el tipo,
                si este se queda sin servicios.
             */
            if(!(ser.size()>0)) _services.remove(type);
    ...
    protected void unregisterService(String owner){
        ...
        while(keys.hasMoreElements()){
            ...
            // Buscamos en la lista si alg�n nombre se corresponde con owner
            i=0;
            end=false;
    ...
    protected void unregisterAllServices(){
       ...
       //_services.clear();        
        while(keys.hasMoreElements()){
            key=keys.nextElement();
            listSer=_services.get(key);
            // Buscamos en la lista si alg�n due�o se corresponde con name
            i=0;
            any=false;
            while(i<listSer.size()){
                ser=listSer.get(i);
                listSer.remove(ser);
                //System.out.println("[SOL] Se  ha eliminado el servicio "+ser.getName()+" cuyo due�o es "+ser.getOwner()
                //                +" y el tipo es "+key);
                i++;
                any = true;
            }
            _services.remove(key);
            if(any) ssv.notifyServicesRemoved();
            ssv.updateServices(_services);
        }
    }
...
//Method to send messages    
    protected void sendMessage(AuroraMessage msg){
        //System.out.println("[SOL] Se env�a un mensaje a ");
        ...
        for(int i=0;i<receivers.size();i++){
            ...
            try{
                if(sc!=null){
                    ...
                    //System.out.println("[SOL] Se env�a un mensaje a "+agent);
                    ssv.addStatusMessage(msg.getSender()+" envia un mensaje a "+agent);
                }
            }catch (Exception e){
                e.printStackTrace();
                // Si hay una excepci�n mandando el mensaje, eliminamos el servicio y el agente de la lista.
