 ...
 * **********************************************************
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ IMPORTANTE El
 * Mensaje AuroraMessage no puede ocupar mas de 1024 bytes
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * **********************************************************
 */
...
public class AuroraMessage {
    ...
    public static AuroraMessage AuroraMessage_Waspmote_Reg(String msg) {
        AuroraMessage aur = new AuroraMessage();
        StringTokenizer st = new StringTokenizer(msg, "$");
        String reg, send;

        reg = st.nextToken();
        String sender = reg;
        reg = reg + "##iMuseumI##sensors";

        //send = st.nextToken();

        Vector<String> rec = new Vector<String>();
        rec.add("HOST");
        aur.setReceivers(rec);
        aur.setContent(reg);
        Random r = new Random(100);
        aur.setConverstionId("ConverstaionId-" + r.nextInt());
        aur.setEnconding("UTF-8");
        aur.setLanguage("ENGLISH");
        aur.setOntology("REG_ONTOLOGY");
        aur.setPerformative(AuroraMessage.ACCEPT_PROPOSAL);
        aur.setProtocol("RegisterAgent");
        aur.setSender(sender);

        return aur;
    }

    public static AuroraMessage AuroraMessage_Waspmote_Data(String msg) {
        AuroraMessage aur = new AuroraMessage();
        StringTokenizer st = new StringTokenizer(msg, "$");
        String reg, send, protoc = "";
        ...
        if (sender.equalsIgnoreCase("NFCSensorWaspmote")) {
            protoc = "NFCProtocol";
        } else {
            if (reg.contains("room1")) {
                protoc = "DataRoomoneProtocol";
            } else if (reg.contains("room2")) {
                protoc = "RoomtwoProtocol";
            }
        }
        ...
        aur.setEnconding("UTF-8");
        aur.setLanguage("ENGLISH");
        aur.setOntology("SensorOntology");
        aur.setPerformative(AuroraMessage.INFORM);
        aur.setSender(sender);
        aur.setProtocol(protoc);

        return aur;
    }

    public AuroraMessage addReceiver(AuroraMessage aur, String dst) {
        if (aur != null) {
            Vector<String> rec = new Vector<String>();
            rec.add(dst);
            aur.setReceivers(rec);
